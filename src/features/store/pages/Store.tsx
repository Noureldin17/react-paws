...
const Store: React.FC = () => {
  ...
  <h2 className="text-2xl font-bold text-primary">Pet Store</h2>
      ...
      >
        ...
        <p className="text-secondary mb-6">Browse our collection of pet supplies.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {data?.pages.map((page, pageIndex) =>
            page?.content.map((product, productIndex) => (
              <ProductCard key={`${pageIndex}-${productIndex}`} product={product} />
            ))
          )}
        </div>
        {isFetchingNextPage && <p className="text-center mt-4">Loading more products...</p>}
      </div>
